...
var Agent = function(opt) {
  ...
  opt = opt || {};
  opt.temporal_window = opt.temporal_window || 1;
  opt.experience_size = opt.experience_size || 30000;
  opt.start_learn_threshold = opt.start_learn_threshold || 1000;
  opt.gamma = opt.gamma || 0.7;
  opt.learning_steps_total = opt.learning_steps_total || 200000;
  opt.learning_steps_burnin = opt.learning_steps_burnin || 3000;
  opt.epsilon_min = opt.epsilon_min || 0.05;
  opt.epsilon_test_time = opt.epsilon_test_time || 0.05;

  var num_inputs = this.eyes.length * 3
  var num_actions = this.actions.length;
  var network_size = num_inputs*opt.temporal_window + num_actions*opt.temporal_window + num_inputs;
  ...
  if (!opt.tdtrainer_options) opt.tdtrainer_options = {}

  if (!opt.tdtrainer_options.hasOwnProperty('learning_rate')) {
    opt.tdtrainer_options['learning_rate'] = 0.001
  }

  if (!opt.tdtrainer_options.hasOwnProperty('momentum')) {
    opt.tdtrainer_options['momentum'] = 0.0
  }

  if (!opt.tdtrainer_options.hasOwnProperty('batch_size')) {
    opt.tdtrainer_options['batch_size'] = 64
  }

  if (!opt.tdtrainer_options.hasOwnProperty('l2_decay')) {
    opt.tdtrainer_options['l2_decay'] = 0.01
  }
  
  // braaain
  this.brain = new deepqlearn.Brain(num_inputs, this.actions.length, opt);
...
function tick () {
  ...
  // Skip drawing if not inside a browser
  if (typeof document === 'undefined') return

  // Otherwise draw when needed
...
function createVisualization () {
    if (document.getElementById('vis_canvas')) return 

    var box = document.getElementById('visualization');
    var visCanvas = document.createElement('canvas');
    visCanvas.setAttribute('id', 'vis_canvas');
    visCanvas.setAttribute('width', '350');
    visCanvas.setAttribute('height', '150');
    var graphCanvas = document.createElement('canvas');
    graphCanvas.setAttribute('id', 'graph_canvas');
    graphCanvas.setAttribute('width', '350');
    graphCanvas.setAttribute('height', '150');
    var netCanvas = document.createElement('canvas');
    netCanvas.setAttribute('id', 'net_canvas');
    netCanvas.setAttribute('width', '700');
    netCanvas.setAttribute('height', '200');
    var canvas = document.createElement('canvas');
    canvas.setAttribute('id', 'canvas');
    canvas.setAttribute('width', '700');
    canvas.setAttribute('height', '500');
    var brainInfo = document.createElement('div');
    brainInfo.setAttribute('id', 'brain_info_div');

    var controls = document.createElement('h1');
    controls.innerText = "controls";
    var goveryfastBtn = document.createElement('button');
    goveryfastBtn.setAttribute('onclick', 'goveryfast()');
    goveryfastBtn.innerText = 'Go very fast';
    var gofastBtn = document.createElement('button');
    gofastBtn.setAttribute('onclick', 'gofast()');
    gofastBtn.innerText = 'Go fast';
    var gonormalBtn = document.createElement('button');
    gonormalBtn.setAttribute('onclick', 'gonormal()');
    gonormalBtn.innerText = 'Go normal speed';
    var goslowBtn = document.createElement('button');
    goslowBtn.setAttribute('onclick', 'goslow()');
    goslowBtn.innerText = 'Go slow';

    var evaluate = document.createElement('h1');
    evaluate.innerText = "evaluate";
    var acceptBtn = document.createElement('button');
    acceptBtn.setAttribute('onclick', 'abort(true)');
    acceptBtn.innerText = 'Accept';
    var rejectBtn = document.createElement('button');
    rejectBtn.setAttribute('onclick', 'abort(false)');
    rejectBtn.innerText = 'Reject';

    
    box.appendChild(visCanvas)
    box.appendChild(graphCanvas)
    box.appendChild(netCanvas)
    box.appendChild(canvas)
    box.appendChild(brainInfo)

    box.appendChild(controls)
    box.appendChild(goveryfastBtn)
    box.appendChild(gofastBtn)
    box.appendChild(gonormalBtn)
    box.appendChild(goslowBtn)

    box.appendChild(evaluate)
    box.appendChild(acceptBtn)
    box.appendChild(rejectBtn)

    // Make the functions available on the window
    global.goveryfast = goveryfast
    global.gofast = gofast
    global.gonormal = gonormal
    global.goslow = goslow
...
module.exports['/pando/1.0.0'] = function (x, cb) {
  ...
  console.log('Creating World')
   w = new World(700, 500); // Same as canvas attributes
