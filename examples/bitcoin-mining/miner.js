...
module.exports['/pando/1.0.0'] = function (x, cb) {
  ...
  var duration = job.duration * 1000 // converted to milliseconds
  ...
  while ((new Date() - start) < duration) {
    var nonce = Math.floor(Math.random() * Math.pow(2, 32)).toString()
    var hash = shajs('sha256').update(nonce).update(block).digest()
    result.attempts++
