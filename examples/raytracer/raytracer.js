...
function main(theta) {
    var raw_data = new Array(4 * w * w);

    var out_idx = 0;

    var cameraSlopeX, cameraSlopeZ;
    var cosValue = cos(theta);
    var sinValue = sin(theta);
    var cameraX = 4.5 * cosValue;
    var cameraZ = 3.5 + 4.5 * sinValue;
    ...
    for (y = h=w/2; y-- > -h;) {
        ...
        for (x = -h; x++ < h;) {
            ...
            for (c = 0; ++c < 4;) {
                ...
                cameraSlopeX =  (-x/w) * sinValue - cosValue;
                cameraSlopeZ = (x/w) * cosValue - sinValue;

                raw_data[out_idx++] = trace_ray([cameraX, 1, cameraZ], [cameraSlopeX, y/w, cameraSlopeZ], 1, w, 2);
            }
