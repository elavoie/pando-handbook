...
For both experiments, each fat-tree node has a maximum of 10 children and the number of values delegated to a single child is limited to `10 * Nb leaf nodes * Delegation factor` of that child (only leaf nodes perform computation) and the delegation factor is `1`.
